...
static inline void _sw_i2c_delay(void) {
    for (volatile uint32_t i = 0; i < SSD1306_SW_I2C_DELAY_COUNT; i++) {
        __NOP();
    }
}

static void _sw_i2c_scl_low(void)  { HAL_GPIO_WritePin(SSD1306_SW_I2C_SCL_Port, SSD1306_SW_I2C_SCL_Pin, GPIO_PIN_RESET); }
static void _sw_i2c_scl_high(void) { HAL_GPIO_WritePin(SSD1306_SW_I2C_SCL_Port, SSD1306_SW_I2C_SCL_Pin, GPIO_PIN_SET);   }
static void _sw_i2c_sda_low(void)  { HAL_GPIO_WritePin(SSD1306_SW_I2C_SDA_Port, SSD1306_SW_I2C_SDA_Pin, GPIO_PIN_RESET); }
static void _sw_i2c_sda_high(void) { HAL_GPIO_WritePin(SSD1306_SW_I2C_SDA_Port, SSD1306_SW_I2C_SDA_Pin, GPIO_PIN_SET);   }
static uint8_t _sw_i2c_sda_read(void) { return HAL_GPIO_ReadPin(SSD1306_SW_I2C_SDA_Port, SSD1306_SW_I2C_SDA_Pin); }

static uint8_t _sw_i2c_write_byte(uint8_t byte) {
    for (uint8_t i = 0; i < 8; i++) {
        if (byte & 0x80) { _sw_i2c_sda_high(); } else { _sw_i2c_sda_low(); }
        _sw_i2c_delay();
        _sw_i2c_scl_high();
        _sw_i2c_delay();
        _sw_i2c_scl_low();
...
void ssd1306_WriteCommand(uint8_t byte) {
    _sw_i2c_sda_high(); _sw_i2c_scl_high(); _sw_i2c_delay();
    _sw_i2c_sda_low();  _sw_i2c_delay();    _sw_i2c_scl_low();
    _sw_i2c_write_byte(SSD1306_I2C_ADDR);
    _sw_i2c_write_byte(0x00);
    _sw_i2c_write_byte(byte);
    _sw_i2c_sda_low();  _sw_i2c_scl_high(); _sw_i2c_delay();
    _sw_i2c_sda_high(); _sw_i2c_delay();
}
...
void ssd1306_WriteData(uint8_t *buffer, size_t buff_size) {
    ...
    _sw_i2c_sda_low();  _sw_i2c_delay();    _sw_i2c_scl_low();
    _sw_i2c_write_byte(SSD1306_I2C_ADDR);
    _sw_i2c_write_byte(0x40);
    for (size_t i = 0; i < buff_size; i++) {
        _sw_i2c_write_byte(buffer[i]);
    }
    _sw_i2c_sda_low();  _sw_i2c_scl_high(); _sw_i2c_delay();
    _sw_i2c_sda_high(); _sw_i2c_delay();
}
